...
bool TargetDetection::targetDetectionProc(cv::Mat src,
                         std::vector<std::vector<cv::Point>> &target_contours,
                         std::vector<struct TargetSize> &target_size_list,std::string win_name)
{
    if(contours.size())
        contours.clear();
    ...
    for(int i = 0; i < contours.size(); i++)
    {
        ...
        cv::approxPolyDP(contours[i],contour_poly,10,0);
        target_size.corners=(int)contour_poly.size();
        if(target_size.corners < rune_cur_size_param->corners_min || target_size.corners > rune_cur_size_param->corners_max)
        {
#ifdef DEBUG_MODE
            std::cout<<rune_cur_size_param->corners_min<<">"<<target_size.corners<<">"<<rune_cur_size_param->corners_max<<std::endl;
            std::ostringstream corners;
            corners << target_size.corners;
            cv::putText(this->ret_4, "corners: " + corners.str(), cv::Point(rot_rect.center) + cv::Point(0, 50), cv::FONT_HERSHEY_COMPLEX, 0.3, cv::Scalar(0, 0, 255), 1, 8);
#endif // DEBUG_MODE
            continue;
        }
#ifdef DEBUG_MODE
        else
        {
            std::ostringstream corners;
            corners << target_size.corners;
            cv::putText(this->ret_4, "corners: " + corners.str(), cv::Point(rot_rect.center) + cv::Point(0, 50), cv::FONT_HERSHEY_COMPLEX, 0.3, cv::Scalar(0, 255, 0), 1, 8);
        }
#endif // DEBUG_MODE
